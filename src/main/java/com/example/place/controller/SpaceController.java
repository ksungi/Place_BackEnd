...
public class SpaceController {
	...
	@PostMapping
	public ResponseEntity<?>createSpace(@AuthenticationPrincipal String userId, @RequestBody SpaceDTO dto){
		try {
	...
	@GetMapping
	public ResponseEntity<?>retrieveSpace(@AuthenticationPrincipal String userId){
	
		List<SpaceEntity> entities = service.retrieve(userId);
		List<SpaceDTO> dtos = entities.stream().map(SpaceDTO::new).collect(Collectors.toList());
	...
	@PutMapping
	public ResponseEntity<?> updateSpace(@AuthenticationPrincipal String userId, @RequestBody SpaceDTO dto){
		try {
			//dto를 이용해 테이블에 저장하기 위한 entity를 생성
			SpaceEntity entity = SpaceDTO.toEntity(dto);
